...
// Download selected watershed as GeoJSON
document.getElementById("downloadGeoJSON").addEventListener("click", () => {
  if (!selectedFeature) {
    alert("No watershed selected.");
    return;
  }
  const blob = new Blob([JSON.stringify(selectedFeature, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "selected_watershed.geojson";
  a.click();
...
document.getElementById("downloadCSV").addEventListener("click", () => {
  ...
  const rows = latestData.map(r => headers.map(h => r[h]).join(","));
